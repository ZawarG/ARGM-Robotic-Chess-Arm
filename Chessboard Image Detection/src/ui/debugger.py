...
def displaySquares(vision):
    ...
    plt.figtext(
        ...
        (
            ...
            f"Light brightness min: {(light_bright-light_bstd):.2f}    |    Dark brightness min: {(dark_bright-dark_bstd):.2f}\n"
            f"Light brightness max: {(light_bright+light_bstd):.2f}    |    Dark brightness max: {(dark_bright+dark_bstd):.2f}"
        ),
        ha='center',
        fontsize=12
    )
